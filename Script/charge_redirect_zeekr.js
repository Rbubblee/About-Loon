...
//   URL 换成银河 api-recharge.geely.com 对应接口，请求头整体替换为
//   银河【原生 AppKey 签名 + token】，请求体补 userId/equipmentId。
//   请求由浏览器自己发出、自己接收（CORS 由 charge_capture_galaxy.js
//   给银河响应补头解决），所以每次打开极氪页面都是实时拉银河数据，
//   完全不需要 $httpClient（绕开 Loon 脚本网络失效问题）。
//
// 为什么 v2 失败而这次可行：v2 用网页端 JS AppKey（204184054）被网关
//   A400IK 拒绝；本版用 evse-hub-ha 开源的原生密钥 204195485 +
//   CqPwP83wzdjesmLeDuzK6SljsYN5PvRM（签名已与抓包逐字节验证一致）。
//
// 依赖：charge_capture_galaxy.js 保存 galaxyRechargeToken/galaxyUserId/
//       galaxyLastEquipmentId/galaxyLastProviderNo（打开银河App时刷新）。
// 降级：token 缺失/过期 → 放行原请求（极氪显示自身页面）并弹通知。
// ============================================================
...
// ---------------- 接口映射（极氪路径 -> 银河接口 + 请求体构造） ----------------
var MAP = {
  "/app/equipment/v2/manage/getMyEquipments": { path: "/gep/v2/home/charge/getMyEquipments", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u }; } },
  "/app/equipment/v2/manage/getMyEquipmentDetail": { path: "/gep/v1/home/charge/getMyEquipmentDetail", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, equipmentId: eq, providerNo: p }; } },
  "/app/equipment/v2/manage/getMyEquipmentCards": { path: "/gep/v2/home/charge/getMyEquipmentCards", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, equipmentId: eq, providerNo: p }; } },
  "/app/equipment/v2/manage/getMyEquipmentShares": { path: "/gep/v1/home/charge/getMyEquipmentShares", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, equipmentId: eq, providerNo: p }; } },
  "/app/equipment/v2/manage/getEquipmentVersions": { path: "/gep/v1/home/charge/getEquipmentVersions", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, equipmentId: eq, providerNo: p }; } },
  "/app/equipment/v2/manage/getEquipmentBindVins": { path: "/gep/v2/home/charge/getEquipmentBindVins", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, equipmentId: eq, providerNo: p }; } },
  "/app/equipment/v2/manage/getEquipmentChargeOrders": { path: "/gep/v2/home/charge/getEquipmentChargeOrders", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, equipmentId: eq, providerNo: p, calcType: 1, pageNum: 1, pageSize: 10 }; } },
  "/app/equipment/v2/manage/getEquipmentChargeOrderCalc": { path: "/gep/v2/home/charge/getEquipmentChargeOrderCalc", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, equipmentId: eq, providerNo: p, calcType: 1 }; } },
  "/app/sim/v1/netflow/generateRenewUrl": { path: "/sim/v1/netflow/generateRenewUrl", build: function (u, eq, p, src) { return { sourceTypeKey: "0010000", userId: u, deviceSn: eq, providerNo: p }; } }
};
...
try {
  ...
  // CORS 预检：直接合成放行响应（浏览器只校验头，不需要真的发到上游）
  if (method === "OPTIONS") {
    if (!rule) { $done({}); return; }
    var origin = String(reqHeaders["origin"] || "https://c-h5-prod.haohanpower.tech");
    var allowHdrs = String(reqHeaders["access-control-request-headers"] || "appid,appuser,authorization,channelid,content-md5,content-type,ctoken,datatheme,device,devicecode,devicetype,language,network,nonce,operator,random1,random2,request-original,requestid,sdkversion,sign-method,signature,sourcecode,system,timestamp,version");
    console.log("[charge] CORS预检 -> 合成放行响应");
    $done({
      response: {
        status: 200,
        headers: {
          ...
          "Access-Control-Allow-Headers": allowHdrs,
  ...
  if (!rule) {
    console.log("[charge] 未映射极氪接口，放行: " + path);
    if (NOTIFY) $notification.post("充电桩修改：未映射接口", path + "（把这条发给我，我来加映射）", "");
    $done({});
    return;
  }
  if (method !== "POST") { $done({}); return; }
  ...
  var srcBody = {};
  try { srcBody = JSON.parse($request.body || "{}"); } catch (e) {}
  var eqId = String(srcBody.equipmentId || srcBody.deviceSn || srcBody.deviceId || eq || "");
  if (!eqId) eqId = eq;
