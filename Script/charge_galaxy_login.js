...
<button id="btnShowCaptcha" style="display:none">滑块没自动弹出？点这里手动弹出</button>
...
// 同源代理：页面直接请求 h5-recharge.geely.com 的真实 API 路径（/api/v1/、/gep/），
// 由 Loon [URL Rewrite]/http-request 脚本改写为真实网关（兼容 header 的两种语义）。
var GALAXY_HOST = "https://h5-recharge.geely.com";
...
function diag(text) {
  ...
  pre.textContent = "[diag] " + text;
  $("out").appendChild(pre);
}

window.onerror = function (msg, src, line, col, err) {
  try {
    showMsg("页面JS异常：" + msg + "（" + (src || "") + ":" + line + "）", "err");
  } catch (e) {}
};

function parseResp(res, text) {
  try { return JSON.parse(text); }
  catch (e) {
    var isHtml = /<html|<head|nginx|Not Found/i.test(text);
    return { code: String(res.status), message: text, _html: !!isHtml, _status: res.status };
  }
}

async function galaxyGet(path, glUserId, token) {
  var url = GALAXY_HOST + path;
  diag("请求安全配置: " + url);
  var res = await fetch(url, { method: "GET", headers: galaxySign("GET", path, "", glUserId, token) });
  var text = await res.text();
  var j;
  try { j = JSON.parse(text); } catch (e) { j = { code: String(res.status), message: text }; }
  j._status = res.status;
  j._raw = String(text).slice(0, 400);
  try { j._relayDebug = res.headers.get("x-relay-debug") || ""; } catch (e) {}
  return j;
}
...
function triggerCaptcha() {
  if (!gtObj) { showMsg("滑块对象尚未就绪，请稍候", "warn"); return; }
  try {
    if (typeof gtObj.verify === "function") { gtObj.verify(); showMsg("已触发滑块弹出，请完成滑动", "warn"); }
    else if (typeof gtObj.showBox === "function") { gtObj.showBox(); showMsg("已触发滑块弹出，请完成滑动", "warn"); }
    else if (typeof gtObj.showCaptcha === "function") { gtObj.showCaptcha(); showMsg("已触发滑块弹出，请完成滑动", "warn"); }
    else { showMsg("滑块对象无可用触发方法（verify/showBox/showCaptcha）", "err"); }
  } catch (e) { showMsg("触发滑块异常：" + (e && e.message ? e.message : String(e)), "err"); }
}
$("btnShowCaptcha").onclick = triggerCaptcha;

async function loadGeetest() {
  setBtn("btnGetCode", true);
  showMsg("① 正在获取安全配置…", "warn");
  var cfg = await galaxyGet("/api/v1/security/config?type=GEE_TEST_V4");
  if (cfg.code !== "success" || !cfg.data) {
    var hint = "";
    ...
    var detail = "status=" + cfg._status + " body=" + (cfg._raw || "");
    if (cfg._relayDebug) { detail += " || relay: " + cfg._relayDebug; }
    showMsg("获取安全配置失败：" + JSON.stringify(cfg).slice(0, 160) + " " + hint + " 【" + detail.slice(0, 300) + "】", "err");
    setBtn("btnGetCode", false);
    return;
  }
  CAPTCHA_ID = cfg.data.captchaId;
  diag("安全配置成功: captchaId=" + CAPTCHA_ID + " apiServer=" + (cfg.data.apiServer || "") + " staticServer=" + (cfg.data.staticServer || ""));
  showMsg("② 安全配置 OK（captchaId=" + CAPTCHA_ID.slice(0, 8) + "…），加载滑块SDK…", "warn");
  // 与 evse-hub-ha 完全一致：固定从 captcha4.geely.com 加载 SDK，apiServers 用裸主机名
  var ok = await loadGeetestSdk();
  if (!ok) {
    showMsg("滑块SDK加载失败：无法访问 https://captcha4.geely.com/www/gt4.js（请检查网络；该域名无需也不应被Loon MITM）", "err");
    setBtn("btnGetCode", false);
    return;
  }
  if (typeof window.initGeetest !== "function") { showMsg("滑块SDK加载失败：gt4.js 已下载但 window.initGeetest 不存在", "err"); setBtn("btnGetCode", false); return; }
  showMsg("③ 正在初始化滑块…", "warn");
  window.initGeetest({
    captchaId: CAPTCHA_ID,
    product: "bind",
    protocol: "https://",
    apiServers: ["captcha4.geely.com"],
    staticServers: ["captcha4.geely.com/www/js/"],
    clientType: "h5",
    hideBar: [],
    onError: function (e) {
      showMsg("滑块初始化失败：" + JSON.stringify(e).slice(0, 180), "err");
      setBtn("btnGetCode", false);
    }
  }, function (captchaObj) {
    geetestReady = true;
    ...
    var doVerify = function () {
      if (triggered) { return; }
      triggered = true;
      showMsg("④ 滑块SDK就绪，正在弹出…", "warn");
      triggerCaptcha();
    };
    if (typeof captchaObj.onReady === "function") { captchaObj.onReady(doVerify); }
    // 兜底：部分 SDK 版本不触发 onReady
    setTimeout(doVerify, 2500);
    // 手动兜底按钮
    $("btnShowCaptcha").style.display = "block";
    captchaObj.onSuccess(function () {
      var r = captchaObj.getValidate();
      if (!r) { showMsg("滑块未返回验证结果", "err"); return; }
      showMsg("滑块验证通过，正在校验…", "ok");
      doValidateAndSend(r);
    });
    captchaObj.onError(function (e) { showMsg("滑块异常：" + JSON.stringify(e).slice(0, 150), "err"); setBtn("btnGetCode", false); });
...
function loadGeetestSdk() {
  return new Promise(function (resolve) {
    if (typeof window.initGeetest === "function") { resolve(true); return; }
    var s = document.createElement("script");
    s.src = "https://captcha4.geely.com/www/gt4.js";
